    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _setPopupMenuItems: function() {
        ...
        this.featureActionManager.getSupportedActions(featureSet, layer).then(lang.hitch(this, function(actions){
          ...
          if(layerConfig.relates && layerConfig.relates && this.list.hasSelectedItem()){
      ...
      onGraphicalLayerChange: function (newValue) {
        ...
        if(this.config.layers[newValue].expressions.length > 0){
      ...
      onAttributeLayerChange: function (newValue) {
        ...
        var layerConfig = this.config.layers[newValue];
        this._initSelectedLayerExpressions();
        var valuesObj = lang.clone(this.config.layers[newValue].expressions[0].values);
        html.empty(this.textsearchlabel);
        if(this.config.layers[newValue].expressions[0].textsearchlabel !== ""){
          html.place(html.toDom(this.config.layers[newValue].expressions[0].textsearchlabel), this.textsearchlabel);
        ...
        this.paramsDijit.build(valuesObj, this.layerInfos[layerConfig.url], this.config.layers[newValue].url,
      ...
      onAttributeLayerExpressionChange: function (newValue) {
        ...
        var layerConfig = this.config.layers[this.AttributeLayerIndex];
        var valuesObj = lang.clone(this.config.layers[this.AttributeLayerIndex].expressions[newValue].values);
        html.empty(this.textsearchlabel);
        if(this.config.layers[this.AttributeLayerIndex].expressions[newValue].textsearchlabel !== ""){
          html.place(html.toDom(this.config.layers[this.AttributeLayerIndex].expressions[newValue].textsearchlabel), this.textsearchlabel);
          html.style(this.textsearchlabel, 'display', 'block');
        }else{
          html.style(this.textsearchlabel, 'display', 'none');
        }
        ...
        this.paramsDijit.build(valuesObj, this.layerInfos[layerConfig.url], this.config.layers[this.AttributeLayerIndex].url,
      ...
      _initBufferUnits: function () {
        ...
        var options = [];
        var len = this.config.bufferDefaults.bufferUnits.length;
        for (var i = 0; i < len; i++) {
          var option = {
            value: this.config.bufferDefaults.bufferUnits[i].name,
            label: this.config.bufferDefaults.bufferUnits[i].label
          };
          options.push(option);
          if (i === 0) {
      ...
      _initSpatialRelationships: function () {
        var len = this.config.spatialrelationships.length;
        for (var i = 0; i < len; i++) {
          var iClass = this._getSpatialIconClass(this.config.spatialrelationships[i].name);
          var spatButton = html.create('div', {
            'class': 'spatial-item',
            'data-spatialtype': this.config.spatialrelationships[i].name,
            'title': this.config.spatialrelationships[i].label
          }, this.spatialItems);
...
//            console.info("spatial layer index: " + this.spatialLayerIndex);
      ...
      _initTabContainer: function () {
        ...
        //determine if this layer has any expressions
        if(this.config.layers[0].expressions.length > 0){
          this.cbxAddTextQuery.setStatus(true);
        }else{
          this.cbxAddTextQuery.setStatus(false);
        }
        ...
        if (this.attribTab) {
          for (var a = 0; a < len; a++) {
            if (this.config.layers[a].expressions.length > 0) {
              tabs.push({
      ...
      _initLayerSelect: function () {
        ...
        for (var i = 0; i < len; i++) {
          ...
          if(this.config.layers[i].expressions.length > 0){
          ...
          all(defs).then(lang.hitch(this, function (results) {
            ...
            array.forEach(results, lang.hitch(this, function (result, j) {
              if(result.state === 'success'){
                ...
                this.layerInfos[this.config.layers[j].url] = layerInfo;
              }else{
                ...
                this.layerInfos[this.config.layers[j].url] = {}
              }
            }));
            ...
            if (myObject.esearch) {
              ...
              if(this.config.layers[myObject.slayer].expressions.length > 0){
                var valuesObj1 = lang.clone(this.config.layers[myObject.slayer].expressions[myObject.exprnum || 0].values);
                ...
                html.empty(this.textsearchlabel);
                if(this.config.layers[myObject.slayer].expressions[myObject.exprnum || 0].textsearchlabel !== ""){
                  html.place(html.toDom(this.config.layers[myObject.slayer].expressions[myObject.exprnum || 0].textsearchlabel), this.textsearchlabel);
                  html.style(this.textsearchlabel, 'display', 'block');
                }else{
                  html.style(this.textsearchlabel, 'display', 'none');
                }
                //this.paramsDijit.build(valuesObj1, this.resultLayers[myObject.slayer], this.config.layers[myObject.slayer].url,
                this.paramsDijit.build(valuesObj1, this.layerInfos[this.config.layers[myObject.slayer].url], this.config.layers[myObject.slayer].url,
              ...
              if(attribOptions.length > 0){
                ...
                this._initSelectedLayerExpressions();
                if(this.config.layers[aIndex].expressions.length > 0){
                  var valuesObj = lang.clone(this.config.layers[aIndex].expressions[0].values);
                  html.empty(this.textsearchlabel);
                  if(this.config.layers[aIndex].expressions[0].textsearchlabel !== ""){
                    html.place(html.toDom(this.config.layers[aIndex].expressions[0].textsearchlabel), this.textsearchlabel);
                    html.style(this.textsearchlabel, 'display', 'block');
                  }else{
                    html.style(this.textsearchlabel, 'display', 'none');
                  }
                  //this.paramsDijit.build(valuesObj, this.resultLayers[aIndex], this.config.layers[aIndex].url,
                  this.paramsDijit.build(valuesObj, this.layerInfos[this.config.layers[aIndex].url], this.config.layers[aIndex].url,
      ...
      _relateResultItem: function(index, item) {
        ...
        var sResult = item;
        this.relArray = [];
        for(var r=0; r < layerConfig.relates.length; r++){
          var relRslt = {
            id: layerConfig.relates[r].id,
            name: layerConfig.relates[r].label,
            fields: layerConfig.relates[r].fields,
        ...
        if(this.attWidget){
          ...
          if(this.relArray.length === 1){
            this._createLayerAndExecuteQuery(this.relArray[0].id);
          }else{
      ...
      _createLayerAndExecuteQuery: function(relateId) {
        var layerConfig = this.config.layers[this.currentLayerIndex];
        var relateIdIndx = this.relArray.findIndex(rel=>{
          return rel.id == relateId
        });
        var relateTableId = parseInt(relateId);
        this._createRelateResultLayer(relateIdIndx).then(lang.hitch(this, function(result){
          ...
          if(this.relArray[relateIdIndx].fields.all){
            relQuery.outFields = ["*"];
          }else{
            var outFields = array.map(this.relArray[relateIdIndx].fields.field, lang.hitch(this, function (fieldInfo) {
              return fieldInfo.name;
            }));
            relQuery.outFields = outFields;
          }
          relQuery.relationshipId = parseInt(relateTableId);
          relQuery.objectIds = [this.relArray[relateIdIndx].oid];
          relQuery.returnGeometry = true;
          var queryTask = new QueryTask(layerConfig.url);
          queryTask.executeRelationshipQuery(relQuery, lang.hitch(this, this._onRelSearchFinish, this.relArray[relateIdIndx].oid, relateIdIndx));
        }));
      },
      ...
      _onRelSearchFinish: function (oid, relateId, result) {
        ...
        if(!result[oid]){
          ...
          new Message({
            ...
            message: this.nls.noRelatedRecords + " " + layerConfig.relates[relateId].label
          });
          return;
        }
        ...
        if(!layerConfig.relates[relateId].fields.all){
          var adjRelFldInfos = [];
      ...
      _createRelateResultLayer: function (relateId) {
        ...
        if(this.relateLayers && this.relateLayers.length > 0){
          array.some(this.relateLayers, lang.hitch(this, function(rLayer){
            if(rLayer.name === this.nls.relate + ': ' + layerConfig.relates[relateId].label){
        ...
        if(!relLyrExists){
          ...
          var relateTableId = layerConfig.relates[relateId].id;
          //var relMsId = this.resultLayers[this.currentLayerIndex].relationships[relateTableId].relatedTableId;
          var relMsId = this.layerInfos[this.config.layers[this.currentLayerIndex].url].relationships[relateTableId].relatedTableId;
          relateLyrUrl = serviceUrl + '/' + relMsId;
          this._getLayerInfoWithRelationships(relateLyrUrl).then(lang.hitch(this, function(result){
            var layerInfo = result.value;
            layerInfo.name = this.nls.relate + ': ' + layerConfig.relates[relateId].label;
            layerInfo._titleForLegend = layerInfo.name;
            layerInfo.minScale = 0;
            layerInfo.maxScale = 0;
            layerInfo.effectiveMinScale = 0;
            ...
            var necessaryFieldNames = array.map(layerConfig.relates[relateId].fields.field, lang.hitch(this, function (fieldInfo) {
              return fieldInfo.name;
            }));
            ...
            if(!layerConfig.relates[relateId].fields.all){
              var adjFieldsOrder = [];
              array.map(layerConfig.relates[relateId].fields.field, lang.hitch(this, function (fieldInfo) {
...
//make sure the form reflects what was searched
        ...
        setTimeout(lang.hitch(this, function(){
          ...
          setTimeout(lang.hitch(this, function(){
            var valuesObj = lang.clone(this.config.layers[slayerId].expressions[exprNum || 0].values);
      ...
      _createSearchResultLayer: function (layerIndex) {
        ...
        var layerInfo = lang.clone(this.layerInfos[this.config.layers[layerIndex].url]);
...
//        console.info(fldName, layerIndex, relateId);
        var layerConfig = this.config.layers[layerIndex].relates[relateId];
        var fields = layerConfig.fields.field;
        var retFldObj = null;
        array.some(fields, lang.hitch(this, function (fieldInfo) {
          if(fieldInfo.name === fldName){
      ...
      _getOutputFields: function (layerIndex, popupFieldName) {
        ...
        var allFieldInfos = this.layerInfos[layerConfig.url].fields;
      ...
      _initSelectedLayerExpressions: function () {
        ...
        var elen = this.config.layers[this.AttributeLayerIndex].expressions.length;
        for (var e = 0; e < elen; e++) {
          var eoption = {
            value: e,
            label: this.config.layers[this.AttributeLayerIndex].expressions[e].alias
          };
      ...
      search: function (geometry, layerIndex, /* optional */ expressIndex, theValue, spatialRelationship, closeOnComplete) {
        ...
        if(this._isServiceSupportsOrderBy(this.layerInfos[this.config.layers[layerIndex].url])){
          //set sorting info
      ...
      checkForRequiredFieldsEntered: function() {
        ...
        for (var s = 0; s < content.length; s++) {
          var spRequired = this.config.layers[this.AttributeLayerIndex].expressions[this.expressIndex].values[s].required || false;
      ...
      clearFields: function () {
        if(this.AttributeLayerIndex || this.AttributeLayerIndex === 0){
          ...
          var valuesObj = lang.clone(this.config.layers[this.AttributeLayerIndex].expressions[exInd].values);
      ...
      buildWhereClause: function (layerIndex, expressIndex, /* optional */ theValue) {
        ...
        for (var s = 0; s < content.length; s++) {
          var tOperator = (this.config.layers[layerIndex].expressions[expressIndex].values[s] &&
            typeof this.config.layers[layerIndex].expressions[expressIndex].values[s].operator !== 'undefined') ? this.config.layers[layerIndex].expressions[expressIndex].values[s].operator : 'OR';
          var tOperation = this.config.layers[layerIndex].expressions[expressIndex].values[s].operation;
          var queryExpr = this.config.layers[layerIndex].expressions[expressIndex].values[s].sqltext;
          if (!content[s].hasOwnProperty('value') || content[s].value === null) {
            if(!content[s].hasOwnProperty('value1') || content[s].value1 === null){
              continue;
            }
          ...
          if (tOperation === 'stringOperatorContains') {
            ...
            if(this.config.containsword){
              ...
              for(w=0; w < sa.length; w++){
                word = sa[w];
                criteriaFromValue = queryExpr.replace(myPattern, word.replace(/'/g, "''"));
                expr = this.AppendTo(expr, criteriaFromValue, tOperator);
              }
              continue;
            }else{
              criteriaFromValue = queryExpr.replace(myPattern, content[s].value.toString().replace(/'/g, "''"));
              expr = this.AppendTo(expr, criteriaFromValue, tOperator);
              continue;
            }
          }
          ...
          if (content[s].value.toString().toLowerCase().trim() === "all") {
            ...
            var uList = this.config.layers[layerIndex].expressions[expressIndex].values[s].userlist;
            var myPat;
            var uaList;
            if (uList.indexOf("','") > -1) {
              myPat = /,\s*'all'/gi;
...
//Change in 2.9 to see if we can allow null values back in the the unique dropdown
      ...
      _getAllLyrFields: function(){
        ...
        var tempFlds = array.filter(this.layerInfos[this.config.layers[this.currentLayerIndex].url].fields, lang.hitch(this, function (fieldInfo) {
      ...
      _onSearchFinish: function (layerIndex, closeOnComplete, removing, adding, results) {
        ...
        if(this.layerInfos[this.config.layers[this.currentLayerIndex].url].type.toLowerCase() !== "table"){
...
//modify the currentFeatures with the new results
        ...
        for (i = 0; i < len; i++) {
          ...
          var qLinks = [];
          if (layerConfig.links && layerConfig.links) {
          ...
          if(layerConfig.showattachments){
            this._addAttachment2Results(featureAttributes[layerConfig.objectIdField]);
          }
          ...
          array.map(tempFlds, lang.hitch(this, function (attr) {
            ...
            if (value !== "") {
              ...
              if (att === typeIdField) {
                ...
                var featureType = this._getFeatureType(this.layerInfos[this.config.layers[layerIndex].url], typeID);
                ...
                var codedValue = this._getCodedValue(this.layerInfos[this.config.layers[layerIndex].url], att, fieldValue, null);
                if (codedValue) {
          ...
          if(!removing){
            ...
            this.list.add({
              ...
              showRelate: layerConfig.relates && layerConfig.relates,
      ...
      _setCurentLayerRenderer: function (symFromWhere) {
        if (symFromWhere === 'server') {
          ...
          return jsonUtil.fromJson(this.layerInfos[this.config.layers[this.currentLayerIndex].url].drawingInfo.renderer);
        } else {
          var symbol,
            type = this.layerInfos[this.config.layers[this.currentLayerIndex].url].geometryType;
      ...
      _selectResultItem: function (index, item) {
        ...
        var FeatLyr = new FeatureLayer(this.layerInfos[this.config.layers[this.currentLayerIndex].url]._origLayerURL);
        var point = item.centerpoint;
        var layerConfig = this.config.layers[this.currentLayerIndex];
        var lyrHasPopupDisabled = (layerConfig.hasOwnProperty("disablePopups") && layerConfig.disablePopups)?true:false;
      ...
      _addAttachment: function(OID) {
        ...
        var ofl = new FeatureLayer(this.layerInfos[this.config.layers[this.currentLayerIndex].url]._origLayerURL);
      ...
      _addAttachment2Results: function(OID) {
        //var ofl = new FeatureLayer(this.resultLayers[this.currentLayerIndex]._origLayerURL);
        var ofl = new FeatureLayer(this.layerInfos[this.config.layers[this.currentLayerIndex].url]._origLayerURL);
        var aIcon, lObj;
        ofl.queryAttachmentInfos(OID, lang.hitch(this, function(info){
          if(info.length > 0){
            var listItem = this.list.items[this._returnListIndexFromOID(OID)];
            array.map(info, lang.hitch(this, function(att){
              if(this.token && att.url.indexOf("?token=") < 0){
                att.url + "?token=" + this.token;
              }
              if(att.contentType.indexOf('image') > -1){
                aIcon = this.folderUrl + 'images/img_icon.png';
              } else if(att.contentType === 'application/pdf'){
                aIcon = this.folderUrl + 'images/pdf_icon.png';
              }else {
                aIcon = this.folderUrl + 'images/i_attachment.png';
              }
              lObj = {
                link: att.url,
                icon: aIcon,
                alias: att.name,
                disableinpopup: true,
                popuptype: this.config.selectAttachmentView || 'text'
              };
              listItem.attachs.push(lObj);
            }));
            this.list.addAttachments(listItem);
          }
        }));
        ofl = null;
      },
