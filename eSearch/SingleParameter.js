    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      _getStringValueObj:function(){
        ...
        if(this._type === 1){
          ...
          valueObj = {};
          var stringCodedItem = this._getSelectedFilteringItem(this.stringCodedValuesFS);
          valueObj.value = stringCodedItem.code;
        }
        return valueObj;
      },
...
//        console.info(valueObj);
      ...
      _getFeatureType:function(typeID) {
        var result;
        this.layerDetails.types.some(type=>{
          if (typeID === type.id) {
            result = type;
            return true
          }
...
//        console.info(valueObj);
        ...
        if(this.layerDetails.typeIdField){
          convert2subtypeVal = (this.layerDetails.typeIdField.toUpperCase() === fieldInfo.name.toUpperCase())
        }
        ...
        if (codedValues || userList || uvField) {
          ...
          if(codedValues && !uvField && !userList){
            stringCodedData = array.map(codedValues, lang.hitch(this, function(item, index) {
              ...
              if(convert2subtypeVal && (dataItem.name !== '' || dataItem.name !== ' ')){
                var featType = this._getFeatureType(dataItem.name);
                if(featType && featType.name){
                  dataItem.name = featType.name;
                }
              }
              dataItem.id = index;
              return dataItem;
