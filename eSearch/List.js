  ...
  function(declare,
    ...
    return declare([_WidgetBase, Evented], {
      ...
      addAttachments: function(item) {
        var itemDom = dojo.byId(this.id.toLowerCase() + item.id);

        var attsDiv;
        if(item.attachs && item.attachs.length > 0 ){
          attsDiv = domConstruct.create("div");
          domConstruct.place(attsDiv, itemDom, "last");
          domClass.add(attsDiv, 'attsdiv');
        }
        var attsLblDiv = domConstruct.toDom("<div>" + this.parentWidget.nls.attachments + ":</div>");
        domConstruct.place(attsLblDiv, attsDiv);
        // console.info(item.links);
        var attsUL = domConstruct.create("ul");
        domClass.add(attsUL, 'attachul');
        
        array.forEach(item.attachs, function(atts, i){
          if(atts.popuptype === "text"){
            if(i === 0) {
              domConstruct.place(attsUL, attsDiv);
            }
            var attsText = domConstruct.toDom("<li><a href='" + atts.link + "' target='_blank' title='" + atts.alias + "'>" + atts.alias + "</a></li>");
            domConstruct.place(attsText, attsUL);
            domClass.add(attsText, 'attlabel');
          }else{
            if(i === 0) {
              var iconSpacerDiv = domConstruct.create("div");
              domClass.add(iconSpacerDiv, 'iconspacer');
              domConstruct.place(iconSpacerDiv, attsDiv);
            }
            var attImg = domConstruct.toDom("<a href='" + atts.link + "' target='_blank' title='" + atts.alias + "'><img src='" + atts.icon + "' alt='" + atts.alias + "' border='0' width='20px' height='20px'></a>");
            domConstruct.place(attImg, attsDiv);
            domClass.add(attImg, 'attIcon');
          }
        });
      },
